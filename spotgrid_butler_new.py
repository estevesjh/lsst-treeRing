...
class SpotgridCatalog():
    ...
    """
    def __init__(self, repository, catalog_collection, calib_collection, sensor='e2v'):
        ...
        # self.filter_spots()
        # self.calibrate()
        ## self.compute_ellipticities()
        ## self.get_imaging_map()    
        ## self.transform_to_treeRing_coords()
...
#            dg2[spot_idx]   = get_perc_error(catalog['ext_shapeHSM_HsmShapeKsb_g2'][select], self.calib_table['ext_shapeHSM_HsmShapeKsb_g2'][spot_idx])
    ...
    def clean2( self, x ):
        ...
        is_good = (self.instFlux_med > 0.1*np.nanmax(self.instFlux_med) )
        
        stdxx=np.nanstd(self.dxx_arr[is_good],axis=0)
...
#        x[:, (stdxx/stdxy>2) | (stdyy/stdxy>2) ] = np.nan
        ...
        for i in range(len(stdxx)):
            ...
            zsel=zorig[finite]
            x=self.x_arr[:,i][finite]
            y=self.y_arr[:,i][finite]
            parameters, covariance = curve_fit(function, [x , y], zsel)
            Z = function(np.array([self.x_arr[:,i], self.y_arr[:,i]]), *parameters)
    ...
    def calibrate(self):
        ...
        self.deltaX  = self.dx_arr[self.spot_filter]
        self.deltaY  = self.dy_arr[self.spot_filter]
        self.deltaR  = np.sqrt(self.deltaX**2+self.deltaY**2)
        
        #subtract "calibrated" grid I_xx+yy (median) moment from each star
        self.deltaXX = self.dxx_arr[self.spot_filter]
        self.deltaYY = self.dyy_arr[self.spot_filter]
        self.deltaXY = self.dxy_arr[self.spot_filter]   
        self.deltaT   = self.deltaXX + self.deltaYY

        self.deltaF  = self.dFlux_arr[self.spot_filter]
        self.deltag1 = self.dg1_arr[self.spot_filter]
        self.deltag2 = self.dg2_arr[self.spot_filter]
