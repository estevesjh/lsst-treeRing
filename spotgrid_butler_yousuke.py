...
#        x[:, (stdxx/stdxy>2) | (stdyy/stdxy>2) ] = np.nan
    ...
    def compute_ellipticities(self):
        ...
        #e0_med = np.nanmedian(e0_total, axis=1)
        #e1_med = np.nanmedian(e1_total, axis=1)
        #e2_med = np.nanmedian(e2_total, axis=1)
        #en_med = np.nanmedian(en_total, axis=1)
        
        #e0_med_reshaped = np.reshape(e0_med[self.spot_filter],(-1,1))
