...
def plot_image_profile_trio(ax, figure, var, ix, highPassFilter=True, color='#8E44AD', yline=1500, width=500, cmap='gray'):
    ...
    ax[1].set_title('Horizontal Profile',fontsize=18)
    ...
    ax[1].set_ylabel(keys2[var][2], fontsize=14)

    #######################
    # vertical component
    ax[2].set_title('Vertical Profile',fontsize=18)
    ax[2].plot(vline, vprofile, color=color, ls='-.')
    # nstd = np.nanstd(vprofile)
    # ax[1].axhspan(-nstd, nstd, color='darkred', alpha=0.3, label='Noise')
    ax[2].axhline(-nstd, color='darkred',ls='--',lw=2)
    ax[2].axhline(+nstd, color='darkred',ls='--',lw=2)

    for li in np.linspace(0,image.shape[0],3):
        ax[2].axvline(li,color='grey',lw=1, ls='--')

    ax[2].set_xlabel('y position [pixels]', fontsize=14)
    ax[2].set_ylabel(keys2[var][2], fontsize=14)
